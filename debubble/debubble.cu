...
__global__ void debubble_sum1_kernel(int*sum, int i){
  int j = i - 1 + i * 2 * (blockIdx.x * blockDim.x + threadIdx.x);
  sum[i + j] += sum[j];
}
__global__ void debubble_sum2_kernel(int*sum, int i){
...
size_t debubble(vector<int> &a) {
  ...
  for (int i = 1; i < n; i *= 2){
    int tot = n / i / 2;
    if(tot < 1024) debubble_sum1_kernel<<<1, tot>>>(s_d, i);
    else debubble_sum1_kernel<<<tot / 1024, 1024>>>(s_d, i);
  }
  for (int i = n / 4; i; i /= 2) {
    int tot = n / i / 2;
    if(tot < 1024) debubble_sum2_kernel<<<1, tot>>>(s_d, i);
    else debubble_sum2_kernel<<<tot / 1024, 1024>>>(s_d, i);
  }
